...
{			
        		...
        		<div class="fsm-wrap-plugin">
                  <!--  <h1>Awsome Social Icons</h1>-->
                    ...
                    ?>
                    <div class="fsm-setting_name">
                   <p>You can use this plugin by using <b>[floatingsocial]</b> shortcode.</p>
                    </div>
                    </form>
                </div>
        <?php
    ...
    {        //Register a setting and its data
		...
		add_settings_field(
		 ...
		 add_settings_field(
		  ...
		  add_settings_section(
            'fsm_setting_section', // ID
            ' Settings', // Title
            array( $this, 'fsm_section_detail' ), // Callback
            'my-setting-admin' // Page
        );  
	...
	if( isset( $input['facebook'] ) )
		...
		if( isset( $input['vine'] ) )
            $new_input['vine'] = sanitize_text_field( $input['vine'] ); 
		 	...
		 	if( isset( $input['select_chk'] ) )
            $new_input['select_chk'] = sanitize_text_field( $input['select_chk'] ); 
			
			if( isset( $input['drop_down'] ) )
            $new_input['drop_down'] = sanitize_text_field( $input['drop_down'] ); 
  ...
  public function fsm_section_detail()
    {
        print 'Choose settings where you want to display social share button ';
    }
	...
	if($options['select_chk']) { $checked = 'checked="checked" '; }
	echo "<input ".$checked." id='select_chk' name='my_option_name[select_chk]' type='checkbox' />";

}	
	public function  dropdown_callback() {
	$options = get_option('my_option_name');
	$items = array("Left", "Right");
	echo "<select id='fsm_drop_down' name='my_option_name[drop_down]' clas='fsm_position'>";
	foreach($items as $item) {
		$selected = ($options['drop_down']==$item) ? 'selected="selected"' : '';
